...
(async () => {
  ...
  const setStateStyles = (
    instance: InstanceNode,
    state,
    style,
    secondaryColor,
    primaryColorAsRgbUnit
  ) => {
    ...
    if (state === 'FOCUS') {
      (instance.strokes = [
        { type: 'SOLID', color: hexToRgbUnitObject(Color(primaryColor).lighten(0.4)) },
      ]),
  ...
  const createButtonsByState = (
    ...
    STATES.forEach((state, i) => {
      ...
      const component = createInstanceComponent({
        mainButton,
        x: i * 300 + instanceX,
        y: instanceY,
        name,
  ...
  const createButtonsByLayout = ({ button, size, yOffset, xOffset, styles }) => {
    ...
    LAYOUTS.forEach((layout) => {
      if (layout === 'WITHOUT_ICON') {
        ...
        const buttons = createButtonsByState(
          button,
          style,
          layout,
          secondaryColor,
