...
document.addEventListener('DOMContentLoaded', () => {
  const initialPrimaryColor = formElements.primaryColorInput.value;
  const initialSecondaryColor = formElements.secondaryColorInput.value;
  const initialBorderRadius = parseInt(formElements.radiusInput.value, 10) || 0;

  const styler = styleButton(formElements.form);
  primaryColorChangeHandler(styler, initialPrimaryColor);
  styler.setSecondaryColor(initialSecondaryColor);
  styler.setRadius(`${initialBorderRadius}px`);
});
...
formElements.form.addEventListener('submit', (e) => {
  ...
  const buttonStyle = Array.from(formElements.radioButtonInputs).find((el) => el.checked).value;
